...
def getprice(species, width, thickness, length=2400):
    ...
    r = requests.post(
        "http://www.timbercut4u.co.uk/shop/ajax.php",
        data={
            "action": "calculate",
            "item_id": species,
            "quantity": 1000,
...
def speciesprices(species):
    return pandas.DataFrame(
        [getprice(species, x, y) for x in widths for y in thicknesses]
    )
...
def make_helpers():
    ...
    s = BeautifulSoup(
        requests.get("http://www.timbercut4u.co.uk/timber").text, features="lxml"
    )
    timber_urls = set(
        [
            x
            for x in [
                x
                for x in [link.get("href") for link in s.findAll("a")]
                if x is not None
            ]
            if "http://www.timbercut4u.co.uk/timber/" in x
        ]
    )
